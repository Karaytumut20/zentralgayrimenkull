...
export default function FloatingSocials() {
  ...
  const phones = {
    konyaMobile: "0501 333 80 42",
    konyaOffice: "0332 233 80 42",
    ankaraMobile: "0532 399 1663"
  };
    ...
    <>
      ...
      {isPhoneModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative z-10 overflow-hidden animate-[in_0.2s_ease-out_fade-in_zoom-in] flex flex-col max-h-[90vh]">
            
            {/* Header */}
            <div className="bg-[#051328] text-white p-4 flex justify-between items-center sticky top-0 z-20">
            ...
            <div className="p-4 overflow-y-auto space-y-6">
              ...
              <div>
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#D4AF37]"></div> KONYA MERKEZ
                </h4>
                <div className="space-y-3">
                    {/* Konya Sabit */}
                    <a 
                        href={`tel:${phones.konyaOffice.replace(/ /g, "")}`}
                        className="group flex items-center gap-4 p-3 rounded-xl border border-gray-100 bg-gray-50 hover:border-blue-500 hover:bg-blue-50 transition-all duration-200"
                    >
                        <div className="w-10 h-10 rounded-full bg-white border border-gray-200 text-blue-700 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors shadow-sm">
                            <Building2 size={20} />
                        </div>
                        <div>
                            <p className="text-[10px] text-gray-500 font-bold uppercase">Ofis Sabit</p>
                            <p className="text-base font-bold text-slate-800">{phones.konyaOffice}</p>
                        </div>
                    </a>

                    {/* Konya Cep */}
                    <a 
                        href={`tel:${phones.konyaMobile.replace(/ /g, "")}`}
                        className="group flex items-center gap-4 p-3 rounded-xl border border-gray-100 bg-gray-50 hover:border-[#D4AF37] hover:bg-yellow-50 transition-all duration-200"
                    >
                        <div className="w-10 h-10 rounded-full bg-white border border-gray-200 text-yellow-700 flex items-center justify-center group-hover:bg-[#D4AF37] group-hover:text-[#051328] transition-colors shadow-sm">
                            <Smartphone size={20} />
                        </div>
                        <div>
                            <p className="text-[10px] text-gray-500 font-bold uppercase">Kurumsal Cep</p>
                            <p className="text-base font-bold text-slate-800">{phones.konyaMobile}</p>
                        </div>
                    </a>
                </div>
              </div>
              ...
              <div className="border-t border-gray-100 pt-4">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-red-500"></div> ANKARA ŞUBE
                </h4>
                
                {/* Ankara Cep */}
                <a 
                    href={`tel:${phones.ankaraMobile.replace(/ /g, "")}`}
                    className="group flex items-center gap-4 p-3 rounded-xl border border-gray-100 bg-gray-50 hover:border-red-500 hover:bg-red-50 transition-all duration-200"
                >
                    <div className="w-10 h-10 rounded-full bg-white border border-gray-200 text-red-700 flex items-center justify-center group-hover:bg-red-600 group-hover:text-white transition-colors shadow-sm">
                        <User size={20} />
                    </div>
                    <div>
                        <p className="text-[10px] text-gray-500 font-bold uppercase">Ahmet Okkalı</p>
                        <p className="text-base font-bold text-slate-800">{phones.ankaraMobile}</p>
                    </div>
                </a>
              </div>
